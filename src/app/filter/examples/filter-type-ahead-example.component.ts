...
@Component({
  ...
  templateUrl: './filter-type-ahead-example.component.html'
})
export class FilterTypeAheadExampleComponent implements OnInit {
  allItems: any[];
  items: any[];
      ...
      fields: [
        {
          ...
          type: 'typeahead',
          queries: [...this.monthQueriesFixed, this.separator, ...this.monthQueries]
        },
        {
          id: 'weekDay',
          title: 'Week Day',
  ...
  filterFieldSelected($event: FilterEvent): void {
    this.filterConfig.fields.forEach(field => {
      if (field.id === 'birthMonth') {
        field.queries = [...this.monthQueriesFixed, this.separator, ...this.monthQueries];
